...
if __name__ == '__main__':
    ...
    for fold_id in range(n_folds):
        ...
        for split in ['train', 'test']:
            graph_data = GraphData(fold_id=fold_id, datareader=datareader, split=split)
        ...
        train_parameters = list(filter(lambda p: p.requires_grad, model.parameters()))
        print('N trainable parameters:', np.sum([p.numel() for p in train_parameters]))
        optimizer = optim.Adam(train_parameters, lr=args.lr, betas=(0.5, 0.999), weight_decay=args.wd)
        ...
        def train(train_loader):
            ...
            for batch_idx, data in enumerate(train_loader):
                ...
                output = model(data)  # when model is gcn_new, and multigraph use this
        ...
        def test(test_loader):
            ...
            for batch_idx, data in enumerate(test_loader):
                for i in range(len(data)):
                    data[i] = data[i].to(args.device)
                # output = model(data[0], data[1])  # when model is gcn baseline or gat, use this
                output = model(data)  # when model is gcn_new, and multigraph use this
    ...
    print(
        '{}-fold cross validation with averge accuracy(+- Standard deviation): {}% ({}%), Recall (+- Standard deviation): {}% ({}%), Precision (+- Standard deviation): {}% ({}%), '
        'F1-Score (+- Standard deviation): {}% ({}%), FPR (+- fpr): {}% ({}%)'.format(
            n_folds, np.mean(accuracy_list), np.std(accuracy_list), np.mean(recall_list), np.std(recall_list),
